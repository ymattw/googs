...
func TestPlayer_Ranking(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			player: Player{Rank: 1040.5},
			want:   "4p",
		},
		{
			name:   "Rank exactly 1037",
			player: Player{Rank: 1037.0},
			want:   "1p",
		},
		{
			name:   "Rank between 30 and 1037",
			player: Player{Rank: 35.0},
			want:   "6.0d",
		},
		{
			name:   "Rank exactly 30",
			player: Player{Rank: 30.0},
			want:   "1.0d",
		},
		{
			name:   "Rank between 1 and 30",
			player: Player{Rank: 25.5},
			want:   "4.5k",
		},
		{
			name:   "Rank less than 1 (negative)",
			player: Player{Rank: -5.0},
			want:   "?",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got != tc.want {
				t.Errorf("For Player with Rank %f, want %q, got %q", tc.player.Rank, tc.want, got)
			}
		})
	}
}
...
func TestTimestamp_UnmarshalJSON(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.wantErr && !ts.Equal(tc.want) {
				t.Errorf("Unmarshal got %v, want %v", ts.Time, tc.want)
			}
		})
	}
}
...
func TestOriginCoordinate_ToA1Coordinate(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("ToA1Coordinate() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !tc.wantErr {
				if got == nil || *got != *tc.want {
					t.Errorf("ToA1Coordinate() = %v, want %v", got, tc.want)
				}
			}
		})
	}
...
func TestNewA1Coordinate(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("NewA1Coordinate() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !tc.wantErr {
				if got == nil || *got != *tc.want {
					t.Errorf("NewA1Coordinate() = %v, want %v", got, tc.want)
				}
			}
		})
	}
...
func TestA1Coordinate_ToOriginCoordinate(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("ToOriginCoordinate() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !tc.wantErr {
				if got == nil || *got != *tc.want {
					t.Errorf("ToOriginCoordinate() = %v, want %v", got, tc.want)
				}
			}
		})
	}
